...
class Jacobi(Clase_Base):
    ...
    def metodo(self):
        ...
        print(f"{'Iter':<5}{'a':<10}{'b':<10}{'c':<10}{'Error 1(%)':<15}{'Error 2(%)':<15}{'Error 3(%)':<15}{'Error(%)':<15}")
        print("-" * 100)
        while True:
            self.iteraciones += 1
            ...
            self.a = (self.sistema[0][3] - (self.sistema[0][1]*x2_old) - (self.sistema[0][2]*x3_old)) / self.sistema[0][0]
            self.b = (self.sistema[1][3] - (self.sistema[1][0]*x1_old) - (self.sistema[1][2]*x3_old)) / self.sistema[1][1]
            self.c = (self.sistema[2][3] - (self.sistema[2][0]*x1_old) - (self.sistema[2][1]*x2_old)) / self.sistema[2][2]


            if self.iteraciones == 1:
                self.error1 =self.error2 = self.error3 = self.error= 100
            else:
                self.error1 = abs((self.a - x1_old) / self.a) * 100
                self.error2 = abs((self.b - x2_old) / self.b) * 100
                self.error3 = abs((self.c - x3_old) / self.c) * 100
